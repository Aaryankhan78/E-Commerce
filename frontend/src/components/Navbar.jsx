  ...
  return (
    <nav className="bg-white dark:bg-gray-900 dark:text-white shadow-sm sticky top-0 z-50 px-6 py-4 transition-colors duration-300">
     ...
     <div className="flex items-center space-x-4">
          ...
          <Link to="/wishlist" className="relative hover:text-indigo-600 dark:hover:text-indigo-400">
            <Heart size={20} />
          </Link>

       
          <Link to="/cart" className="relative hover:text-indigo-600 dark:hover:text-indigo-400">
          ...
          {user ? (
            <div className="hidden sm:flex items-center space-x-2 text-sm">
              <span className="text-gray-700 dark:text-gray-300">Hi, {user.email}</span>
              <button
      ...
      {isOpen && (
        <div className="md:hidden mt-4 flex flex-col space-y-3 font-medium text-gray-700 dark:text-gray-200 animate-fade-in">
          ...
          {user ? (
            ...
            <Link to="/register" onClick={() => setIsOpen(false)} className="hover:text-indigo-600 dark:hover:text-indigo-400">
              Register
            </Link>
          )}
        </div>
